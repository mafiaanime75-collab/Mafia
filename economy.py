...
async def cmd_shop(message: Message):
    await message.answer(render_shop_text())


@router.callback_query(F.data == "menu_shop")
async def on_menu_shop(callback: CallbackQuery):
    await callback.message.answer(render_shop_text())
    await callback.answer()


@router.message(Command("gemshop"))
async def cmd_gemshop(message: Message):
    await message.answer(render_gemshop_text())
